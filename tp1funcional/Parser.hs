...
action_4 (12) = happyShift action_6
...
happyReduction_5 (HappyTerminal (TConst happy_var_1))
	 =  HappyAbsSyn4
		 (Const happy_var_1
	)
...
happyReduction_7 ((HappyAbsSyn4  happy_var_6) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (IfZero happy_var_2 happy_var_4 happy_var_6
	) `HappyStk` happyRest
...
happyNewToken action sts stk (tk:tks) =
	...
	TThen -> cont 9;
	TElse -> cont 10;
	TConst happy_dollar_dollar -> cont 11;
	TId happy_dollar_dollar -> cont 12;
	TLParen -> cont 13;
	TRParen -> cont 14;
	TAdd -> cont 15;
	TSub -> cont 16;
...
tokenParser tks = happyRunIdentity happySomeParser where
...
lexNum cs = TConst (read num) : lexer rest
      where (num,rest) = span isDigit cs
lexVar cs =
   case span isAlpha (map toLower cs) of
      ("let",rest) -> TLet : lexer rest
      ("in",rest)  -> TIn : lexer rest
      ("ifzero",rest)  -> TIfZero : lexer rest
      ("then",rest)  -> TThen : lexer rest
      ("else",rest)  -> TElse : lexer rest
      (id,rest)   -> TId id : lexer rest
